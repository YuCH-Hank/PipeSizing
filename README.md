...
- **風量/風速計算**：可設定風速 (m/s) 與吸風口風量 (m³/s)，連結點自動彙總前端風量；管徑以 `面積 = 風量 / 風速` 計算圓管直徑或方管邊長。
...
1. 以瀏覽器開啟 `index.html`。
2. 右鍵畫布選擇要新增的節點類型。
3. 點擊吸風口即可設定風量；再次點擊選取節點，接著點擊另一個節點後在彈窗輸入多段距離以建立管路。
4. 透過上方控制列設定風速、管徑形狀 (圓管/方管) 與比例尺。
...
## 計算說明
- **風量累計**：吸風口使用者輸入的風量為基礎；連結點及出風口會累計所有進入該節點的管路流量。
- **管徑公式**：`面積 = 風量 / 風速`；圓管直徑 `D = sqrt(4A/π)`，方管假設為正方形邊長 `S = sqrt(A)`。
